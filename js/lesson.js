...
function render(){
 ...
 else c.innerHTML=`<article class="card"><div class="eyebrow">Exercise</div><h2>${x.prompt}</h2>${x.speak?`<div class="speak-line">${audio(x.speak)}</div>`:""}${x.ipaHint?`<div class="hint">${escapeHtml(x.ipaHint)}</div>`:""}<input id="writeInput" class="write-input" placeholder="Type your answer…" autocomplete="off" autocapitalize="none" spellcheck="false"></article>`;
