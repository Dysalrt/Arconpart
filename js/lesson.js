...
function render(){
 ...
 if(x.type==="T")c.innerHTML=`<article class="card"><div class="eyebrow">Theory</div><h2>${x.title}</h2><div class="theory-body">${x.theory}</div><div class="examples">${x.examples.map(e=>`<div class="example"><div class="example-word">${escapeHtml(e.word)} ${audio(e.word)}</div>${e.ipa?`<div class="example-meta ipa-audio">${audio(e.word)}</div>`:""}<div>${escapeHtml(e.translation)}</div></div>`).join("")}</div></article>`;
 ...
 else c.innerHTML=`<article class="card"><div class="eyebrow">Exercise</div><h2>${x.prompt}</h2>${x.speak?`<div class="speak-line">${escapeHtml(x.speak)} ${audio(x.speak)}</div>`:""}${x.ipaHint?`<div class="hint ipa-audio">${x.speak?audio(x.speak):"🔊"}</div>`:""}<input id="writeInput" class="write-input" placeholder="Type your answer…" autocomplete="off" autocapitalize="none" spellcheck="false"></article>`;
